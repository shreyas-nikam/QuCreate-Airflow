...
def get_entry_from_mongo_step(entry_id, **kwargs):
    ...
    entry = mongo_client.find("in_deliverables_generation_queue", filter={"_id": ObjectId(entry_id)})
    if not entry:
...
def update_module_with_deliverables_step(course_id, module_id, video_link, assessment_link, chatbot_link, has_chatbot, has_assessment, slide_link, **kwargs):
    update_module_with_deliverables(course_id, module_id, video_link, assessment_link, chatbot_link, has_chatbot, has_assessment, slide_link)
...
with DAG(
    ...
    def branch_logic(**kwargs):
        ...
        # If neither should run, skip to the join point
        return next_tasks if next_tasks else "join_task"
    ...
    get_resources_link_task = PythonOperator(
        ...
        op_args=["{{ task_instance.xcom_pull(task_ids='get_entry_from_mongo')[0] }}", "{{ task_instance.xcom_pull(task_ids='get_entry_from_mongo')[1] }}"],
        provide_context=True
    )
    ...
    download_slide_task = PythonOperator(
        ...
        op_args=["{{ task_instance.xcom_pull(task_ids='get_entry_from_mongo')[1] }}", "{{ task_instance.xcom_pull(task_ids='get_resources_link')[0] }}"],
        provide_context=True
    )
    ...
    get_slide_content_task = PythonOperator(
        ...
        op_args=["{{ task_instance.xcom_pull(task_ids='get_resources_link')[1] }}"],
        provide_context=True
    )
    ...
    create_images_task = PythonOperator(
        ...
        op_args=["{{ task_instance.xcom_pull(task_ids='get_entry_from_mongo')[1] }}", "{{ task_instance.xcom_pull(task_ids='download_slide') }}"],
        provide_context=True
    )
    ...
    create_audio_task = PythonOperator(
        ...
        op_args=["{{ task_instance.xcom_pull(task_ids='get_entry_from_mongo')[1] }}", "{{ task_instance.xcom_pull(task_ids='get_entry_from_mongo')[2] }}", "{{ task_instance.xcom_pull(task_ids='get_transcript') }}"],
        provide_context=True
    )
    ...
    branch_task >> Label("Skip Both") >> join_task
    
    # Ensure all tasks flow into the join_task before uploading files
    create_videos_task >> join_task
    generate_assessment_task >> join_task
    generate_chatbot_task >> join_task

    # Ensure upload_files_task runs only after all preceding tasks
    join_task >> upload_files_task

    upload_files_task >> update_module_with_deliverables_task >> end