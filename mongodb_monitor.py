...
if __name__ == "__main__":
    main()
  # Update if MongoDB runs on a different host/port
DATABASE_NAME = "test"
COLLECTION_NAME = "in_content_generation_queue"

# Airflow REST API Details
AIRFLOW_API_URL = "http://localhost:8080/api/v1/dags/mongo_new_entry_dag/dagRuns"
AIRFLOW_AUTH = ("admin", "admin")  # Replace with your Airflow username/password

def trigger_airflow_dag(entry_id):
    """Trigger an Airflow DAG run via REST API."""
    response = requests.post(
        AIRFLOW_API_URL,
        auth=AIRFLOW_AUTH,
        json={"conf": {"entry_id": entry_id}},  # Pass the entry ID as a parameter
    )
    if response.status_code == 200:
        print(f"Successfully triggered Airflow DAG for entry_id: {entry_id}")
    else:
        print(f"Failed to trigger Airflow DAG: {response.status_code}, {response.text}")

def main():
    """Watch MongoDB for changes and trigger Airflow DAG."""
    client = MongoClient(MONGO_URI)
    db = client[DATABASE_NAME]
    collection = db[COLLECTION_NAME]
    ...
    # Watch the collection for inserts
    with collection.watch() as stream:
        for change in stream:
            if change["operationType"] == "insert":
                entry_id = str(change["documentKey"]["_id"])  # Extract the ID of the new document
                print(f"New entry detected: {entry_id}")
                trigger_airflow_dag(entry_id)
